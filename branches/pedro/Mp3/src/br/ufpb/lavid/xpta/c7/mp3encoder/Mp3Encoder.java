package br.ufpb.lavid.xpta.c7.mp3encoder;
...
public class Mp3Encoder {
	...
	// currently, there is no convenient method in Java Sound to specify
	// non-standard Encodings.
	// using tritonus' proposal to overcome this.
	private static final AudioFormat.Encoding MPEG1L3 = Encodings
			.getEncoding("MPEG1L3");
	private static final AudioFileFormat.Type MP3 = AudioFileTypes.getType(
			"MP3", "mp3");
	

	/**
	 * Method to run the encode as if it were a stand-alone java executable to
	 * encode ONE audio file to mp3 format at a time.
	 * 
	...
	public static void encode(String args[]) {
		...
		if (args.length > 1)
		{
			for (int i = 1 ; i < args.length ; i++)
			{
				...
				// Parameters with values, quality and bitrate. You get the value of the next argument, therefore args[++i]: 
				else if (cArg =='q')
				{	
					try {
						System.setProperty("tritonus.lame.quality", args[++i]);
					} catch (Throwable t2) {
				...
				{
					try {
						System.setProperty("tritonus.lame.bitrate", args[++i]);
					} catch (Throwable t3) {
	...
	/*
	 * first version. Remains here for documentation how to get a stream with
	 * complete description of the target format.
	 */
	public static AudioInputStream getConvertedStream2(
			AudioInputStream sourceStream, AudioFormat.Encoding targetEncoding)
			throws Exception {
		...
		AudioFormat targetFormat = new AudioFormat(targetEncoding, sourceFormat
				.getSampleRate(), AudioSystem.NOT_SPECIFIED, sourceFormat
				.getChannels(), AudioSystem.NOT_SPECIFIED,
		...
		targetStream = AudioSystem.getAudioInputStream(targetFormat,
				sourceStream);
		...
		if (!quiet) {
			if (DEBUG) {
				System.out.println("Got converted AudioInputStream: "
						+ targetStream.getClass().getName());
			}
	...
	public static int writeFile(String inFilename) {
		...
		try {
			...
			AudioInputStream audioInputStream = getAudioInputStream(inFilename);
			audioInputStream = getConvertedStream(audioInputStream, MPEG1L3);
			System.out.println("Koko da yo");
			// construct the target filename
			String outFilename = stripExtension(inFilename) + "."
					+ targetType.getExtension();

			// write the file
			if (!quiet) {
				System.out.println("Writing " + outFilename + "...");
			}
			writtenBytes = AudioSystem.write(audioInputStream, targetType, new File(
					outFilename));
			if (DEBUG && !quiet) {
				System.out.println("Effective parameters of output file:");
				try {
					String version = System.getProperty(
							"tritonus.lame.encoder.version", "");
					if (version != "") {
					...
					System.out
							.println("  Channel Mode = "
									+ System.getProperty(
											...
											"<none>"));
					System.out.println("  VBR mode     = "
							+ System.getProperty("tritonus.lame.effective.vbr",
									"<none>"));
	...
	public static int parseArgs(String[] args) {
		...
		try {
			for (int i = 0; i < args.length; i++) {
				...
				if (args.length < i + 2) {
					throw new Exception(
							"Missing parameter or unrecognized option " + arg
									+ ".");
				}
	...
	public static void main(String[] args) {
		...
		for (int i = firstFileIndex; i < args.length; i++) {
			...
			if (bytes >= 0) {
				...
				if (!quiet) {
					System.out.println("Wrote " + bytes + " bytes in "
							+ (time / 60000) + "m " + ((time / 1000) % 60)
							+ "s " + (time % 1000) + "ms (" + (time / 1000)
							+ "s).");
				}
			}
		}
		...
		if ((DEBUG && quiet) || !quiet) {
			...
			System.out.println("From " + inputFiles + " input file"
					+ (inputFiles == 1 ? "" : "s") + ", " + success + " file"
					+ (success == 1 ? " was" : "s were")
					+ " converted successfully in " + (totalTime / 60000)
					+ "m " + ((totalTime / 1000) % 60) + "s  ("
					+ (totalTime / 1000) + "s).");
		}
		System.exit(0);
	}
	...
	public static void usage() {
		System.out
				.println("Mp3Encoder - convert audio files to mp3 (layer III of MPEG 1, MPEG 2 or MPEG 2.5");
		System.out
				.println("java Mp3Encoder <options> <source file> [<source file>...]");
		System.out
				.println("The output file(s) will be named like the source file(s) but");
		System.out.println("with mp3 file extension.");
		System.out.println("");
		System.out
				.println("You need LAME 3.88 or later. Get it from http://sourceforge.net/projects/lame/");
		System.out.println("");
		System.out.println("<options> may be a combination of the following:");
		System.out
				.println("-q <quality>  Quality of output mp3 file. In VBR mode, this affects");
		System.out
				.println("              the size of the mp3 file. (Default middle)");
		System.out
				.println("              One of: lowest, low, middle, high, highest");
		System.out
				.println("-b <bitrate>  Bitrate in KBit/s. Useless in VBR mode. (Default 128)");
		System.out
				.println("              One of: 32 40 48 56 64 80 96 112 128 160 192 224 256 320 (MPEG1)");
		System.out
				.println("              Or: 8 16 24 32 40 48 56 64 80 96 112 128 144 160 (MPEG2 and MPEG2.5");
		System.out
				.println("-V            VBR (variable bit rate) mode. Slower, but potentially better");
		System.out.println("              quality. (Default off)");
		System.out.println("-v            Be verbose.");
		System.out.println("-s            Be silent.");
		System.out
				.println("-e            Debugging: Dump stack trace of exceptions.");
		System.out
				.println("-t            Debugging: trace execution of converters.");
